...
public class PrimaryAdminsWindow extends javax.swing.JFrame {
    ...
    private void deletegroupActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_deletegroupActionPerformed
        ...
        if (confirmation == JOptionPane.YES_OPTION) {
            // Delete the group
            groupNotification.handleAction("forAll", user.getUserId(), group.getGroupName()+": the group has been deleted", group.getGroupName());
    ...
    private void changecoverphotoActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_changecoverphotoActionPerformed
        ...
        if (result == JFileChooser.APPROVE_OPTION) {
            ...
            group.saveToFile();
            groupNotification.handleAction("forAll", user.getUserId(), group.getGroupName()+": New cover photo is added to the group", group.getGroupName());
            window.reloadGroupDetails();
    ...
    private void changenameActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_changenameActionPerformed
        ...
        if (name != null && !name.trim().isEmpty()) {
            groupNotification.handleAction("forAll", user.getUserId(), name + ": the group name changed from: " + group.getGroupName() + " to: " + name, group.getGroupName());
    ...
    private void changedescriptionActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_changedescriptionActionPerformed
        ...
        if (description != null && !description.trim().isEmpty()) {
            groupNotification.handleAction("forAll", user.getUserId(), group.getGroupName()+": the description of the group is changed", group.getGroupName());
        ...
        if (result == JOptionPane.OK_OPTION) {
            ...
            if (selectedIndex != -1) {
                ...
                if (group.isMember(selectedMember.getUsername())) {
                    ...
                    group.saveToFile();
                    groupNotification.handleAction("forUser", db.loadUserByName(selectedMember.getUsername()).getUserId(), group.getGroupName()+": You have been promoted to admin!", group.getGroupName());
        ...
        if (result == JOptionPane.OK_OPTION) {
            ...
            if (selectedIndex != -1) {
                ...
                if (group.isAdmin(selectedAdmin.getUsername())) {
                    ...
                    group.saveToFile();
                    groupNotification.handleAction("forUser", db.loadUserByName(selectedAdmin.getUsername()).getUserId(), group.getGroupName()+": You have been demoted to a member", group.getGroupName());
